...
def extract():
    ...
    json_object["hotel_name"]=hotel_name.replace("\n","")
    address=dom.xpath(address)[0].text
    json_object["hotel_address"]=address.replace("\n","")
    start_hotel=dom.xpath(star_class)
    rating=start_hotel[0].get('class').split()
    star_classification=rating[2]
    json_object["hotel_star_classification"]=star_classification
    ...
    hotel_desc=desc_str
    json_object["hotel_description"]=hotel_desc
    review=dom.xpath(review)[0].text
    json_object["hotel_review"]=review
    ...
    for i in room_categories:
        if (room_categories_str==""):
            ...
            room_categories_str=room_categories_str+" , "+i.xpath("string()")
    json_object["hotel_room_categories"]=room_categories_str.replace("\n"," ")
    
    review_count=dom.xpath(review_count)
    json_object["hotel_review_count"]=review_count[1]
    ...
    json_object["hotel_alternate_hotels"]=alternate_hotels_str.replace("\n"," ")
